...
void recursiveGreedyActivitySelector(int q, int k, int n){s
    int m = k + 1;
    while(m <= n && job[m].start < job[k].end)
        ++m;

    if(m <= n){
        selectedJobs[q] = job[m].c;
        //printf("%c\n", selectedJobs[q]);
        ++q;
        recursiveGreedyActivitySelector(q, m, n);
    }
}
...
int main(){
    ...
    */
    ...
    selectedJobs[0] = job[0].c;
    recursiveGreedyActivitySelector(1, 0, N-1);
